...
describe('total likes', () => {
  ...
  test('when list is empty, equal zero likes', () => {
    const result = listHelper.totalLikes([])
    assert.strictEqual(result, 0)
  })

  test('when list has more than one blog, equal total likes of all blogs', () => {
...
describe('favourite blog', () => {
  test('return blog with most likes with unique number of likes', () => {
    const result = listHelper.favouriteBlogs(blogs)
    assert.deepStrictEqual(result, blogs[2])
  })

  test('when list is empty, return empty list', () => {
